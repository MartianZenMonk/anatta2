...
def ledc(c='', f='16hz'):
	...
	if hrs < 6:
		...
		randC = random.choice(["o","dg","g","b","y","p"])
		led_color = randC
		led_color2 = 'r'
	else:
		led_color = random.choice(["r","off"])
		led_color2 = 'off'
...
def speakThaiWord(texts,vol='1000'):
	stext = texts.split(' ')
	for tx in stext:
		try:
			os.system('mpg123 -f ' + vol + ' ' + "../voices/thai/" + tx + ".mp3")
		except:
	...
	if have_internet():
		...
		if len(todayholyday) >0:
			if '8' in todayholyday:
				...
				speakThai_mp3(['วัน','พระ','เล็ก'])
				clear_q()
				return 8
			else:
				speak("One nee one patimok, today is Big Buddha day")
				speakThai_mp3(['วัน','พระ','ใหญ่'])
				clear_q()
				return 15
		else:
			clear_q()
...
def breathing2(c='off', t=30, vol='2000'):

	ledc(c)

	if t==0:
		proc = subprocess.Popen(["mpg123","-d","1","-f",vol,"-q","--loop","-1","../sound/lomhaijai.mp3"])
		press_for_stop(c,proc)
	else:
		proc = subprocess.Popen(["mpg123","-d","1","-f",vol,"-q","--loop","-1","../sound/lomhaijai.mp3"])
		delay(t)
...
def dokmai(t=30, vol='2000',c='off'):

	ledc(c)

	if t==0:
		proc = subprocess.Popen(["mpg123","-d","1","-f",vol,"-q","--loop","-1","../sound/dokmai.mp3"])
		press_for_stop(c,proc)
	else:
		proc = subprocess.Popen(["mpg123","-d","1","-f",vol,"-q","--loop","-1","../sound/dokmai.mp3"])
		delay(t)
...
def bell30sec(t=30, vol='1000',c='off'):

	ledc(c)

	if t==0:
		proc = subprocess.Popen(["mpg123","-d","1","-f",vol,"-q","--loop","-1","../sound/bell-5min.mp3"])
		press_for_stop(c,proc)
	else:
		proc = subprocess.Popen(["mpg123","-d","1","-f",vol,"-q","--loop","-1","../sound/bell-5min.mp3"])
		delay(t)
...
def bell_5minutes(l='1',vol='500'):
	subprocess.run(["mpg123","-q","-f",vol,"--loop",l,"../sound/bell-5min.mp3"])
	return None
...
def relax_walk(t=5,vol="20%"):
	call(["amixer","-q","-M","sset","Master",vol])
	text  = ["พุท","โธ","พุท","โธ","เหยียบ","เหยียบ","รู้","เท้า","เคลื่อน","ไหว","รู้","ใจ","นึก","คิด","มี","จิต","เบิก","บาน","อยู่","กับ","ปัจ","จุ","บัน","หนอ"]
	text += ["พุท","โธ","พุท","โธ","เหยียบ","เหยียบ","ถอน","ความ","พอ","ใจ","และ","ความ","ไม่","พอ","ใจ","ใน","ใจ","ออก","เสีย","ได้"]
	text += ["พุท","โธ","พุท","โธ","เหยียบ","เหยียบ","มี","สติ","คิด","ก่อน","พูด","หนอ","มี","สติ","คิด","ก่อน","พูด","หนอ"]
	text += ["พุท","โธ","พุท","โธ","เหยียบ","เหยียบ","ถูก","ต้อง","พอ","ใจ","ถูก","ต้อง","พอ","ใจ"]
...
def relax_walk2(ts=5,vol="20%"):
	call(["amixer","-q","-M","sset","Master",vol])
	t  = 'พุท โธ พุท โธ เหยียบ เหยียบ มี ความ รู้ สึก ตัว รู้ ลม หาย ใจ รู้ กาย เคลื่อน ไหว รู้ ใจ นึก คิด มี จิต เบิก บาน อยู่ กับ ปัจ จุ บัน '
	t += 'พุท โธ พุท โธ เหยียบ เหยียบ ถอน ความ พอ ใจ และ ความ ไม่ พอ ใจ ใน โลก ออก เสีย ได้ '
	t += 'พุท โธ พุท โธ เหยียบ เหยียบ ถูก ต้อง พอ ใจ ถูก ต้อง พอ ใจ ' 
	t += 'พุท โธ พุท โธ เหยียบ เหยียบ มี สติ คิด ก่อน พูด หนอ มี สติ คิด ก่อน พูด หนอ'
	text = t.split(' ')
	tx   = thaiwords2(text)
	tx_list = tx.split(' ')
	# print(tx_list)
...
def musk_walk(ts=5,vol="40%"):
	call(["amixer","-q","-M","sset","Master",vol])
	t  = 'พุท โธ พุท โธ ความ เห็น ชอบ สัม มา ทิฏ ฏิ ความ รู้ ใน ทุกข์ ความ รู้ ใน เหตุ ให้ เกิด ทุกข์ ความ รู้ ใน ความ ดับ แห่ง ทุกข์ ความ รู้ ใน ทาง ดำ เนิน ให้ ถึง ความ ดับ แห่ง ทุกข์ '
	t += 'พุท โธ พุท โธ ความ ดำริ ชอบ สัม มา สัง กัป โป ดำริ ใน การ ออก จาก กาม ดำริ ใน การ ไม่ มุ่ง ร้าย ดำริ ใน การ ไม่ เบียด เบียน '
	t += 'พุท โธ พุท โธ การ พูด จา ชอบ สัม มา วา จา เว้น จาก การ พูด ไม่ จริง เว้น จาก การ พูด ส่อ เสียด เว้น จาก การ พูด หยาบ เว้น จาก การ พูด เพ้อ เจ้อ '
	t += 'พุท โธ พุท โธ การ ทำ การ งาน ชอบ สัม มา กัม มัน โต เว้น จาก การ ฆ่า เว้น จาก การ ถือ เอา สิ่ง ของ ที่ เจ้า ของ ไม่ ได้ ให้ เว้น จาก การ ประพฤติ ผิด ใน กาม '
	t += 'พุท โธ พุท โธ การ เลี้ยง ชี วิต ชอบ สัม มา อา ชี โว ไม่ ทำ อา ชีพ ทุ จริต ทำ อา ชีพ สุ จริต '
	t += 'พุท โธ พุท โธ ความ เพียร ชอบ สัม มา วา ยา โม ไม่ ทำ ชั่ว ใหม่ เลิก ทำ ชั่ว ที่ ยัง ทำ อยู่ ทำ ความ ดี เพิ่ม รัก ษา ความ ดี ที่ ทำ ไว้ '
	t += 'พุท โธ พุท โธ ความ ระ ลึก ชอบ สัม มา สติ มี สติ ใน กาย มี สติ ใน เว ทะ นา มี สติ ใน จิต มี สติ ใน ธรรม มี ความ เพียร เผา กิเลส มี ความ รู้ สึก ตัว มี สติ ถอน ความ พอ ใจ และ ความ ไม่ พอ ใจ ใน ใจ ออก เสีย ได้ '
	t += 'พุท โธ พุท โธ ความ ตั้ง ใจ มั่น ชอบ สัม มา สมา ธิ เข้า ถึง ปฐม ฌาน มี วิตก วิจาร ปีติ สุข เอกัคคตา เข้า ถึง ทุติย ฌาน ไม่ มี วิตก วิจาร มี แต่ ปีติ สุข เอกัคคตา '
...
def remind_breathing2(t=30,vol='500'):
	#bell('3',vol)
	text  = ["ตั้ง","กาย","ตรง","มี","ความ","รู้","สึก","ตัว","ไม่","หลับ","ไม่","หลง","คิด","ปรุง","แต่ง","ไร้","สาระ","ดอก","ไม้","ดอก","ไม้"]
	tx   = thaiwords2(text)
	timeout = time.time() + 60*t   
	while True:
		if time.time() > timeout:
...
def remind_breathing3(t=30,vol='500'):
	#bell('3',vol)
	text  = ["ดอก","ไม้","ดอก","ไม้","ดอก","ไม้","ดอก","ไม้","ดอก","ไม้","ดอก","ไม้","ดอก","ไม้"]
	tx   = thaiwords2(text)
	timeout = time.time() + 60*t   
	while True:
		if time.time() > timeout:
...
def remind_walking2(t=30,vol='600',n=0):
	...
	if n == 1:
		...
		text = [["พุท"],["โธ"],["พุท"],["โธ"],["รู้"],["ก่อน"],["เคลื่อน"],["ไหว"]]
	elif n == 4:
...
def dhamma_art(t=0,fp='../mars/',logpath="../mars/log/log-art.txt",gain='0.1',rate='1.75'):
	files = [
	['bdd','8021.mp3'],
	['bdd','8022.mp3'],
	['bdd','8023.mp3'],
...
def play_vlc_file(fp="../mars/muttothai.m4a",rate='1.50',gain='0.10'):
	...
	proc = subprocess.Popen(cmd, stdout=subprocess.PIPE, shell=True, stdin=master)
	press_for_stop(led_color2,proc,t)
...
def pali_chanting():
	...
	proc = subprocess.Popen(["mpg123","--loop","-1","-f","3000","../mars/monk/pali.mp3"])
	press_for_stop('d',proc)
...
def umong_testing_4():
	...
	if now < 3:
		...
		mn = x_minutes(3)
		if mn > 30:
			wooden_gong_sound(mn-27,'400','off')
		...
		bell('3')
		play_vlc_file("../mars/uttayandokmai.mp3",'1.00')
		relax_thai()
		dokmai(24,'500')
		remind_breathing3(1,'500')
		dokmai(25,'500')
		bell('1')
		...
		if now < 19:
			...
			if a == 0:
				...
				singing_bowl(5)
				nava = ["../mars/monk/nava_dhamma_chanting.mp3","../mars/monk/nava_vinai.mp3"]
				play_vlc_file(random.choice(nava),'2.00')
				bell('1')
				bd = ["../mars/gold","../mars/12paticca","../mars/4nt"]
				play_vlc_all_inTime(random.choice(bd),40,'1.50','0.1',1)
				bell('1')
				adjust_volume()
				play_vlc_file("../datath/chanting/paticca.mp3",'1.00')
				i = weekday()%2
				mn = 90
				if i == 0:
					i = random.randint(0,1)
					if i == 0:
						st = ["../mars/sutta7","../mars/milin","../mars/bhadde"]
						play_vlc_all_inTime(random.choice(st),mn,'1.50','0.1',1)
						# dm = [["../mars/zen","1.50","0.10"],["../mars/zen2","1.75","0.075"]]
						# dmc = random.choices(dm,[2,1])[0]
						# play_vlc_all_inTime(dmc[0],mn,dmc[1],dmc[2],1)
					else:
						st = ["../mars/sutta","../mars/sutta3","../mars/sutta4"]
						# st = ["../mars/sutta","../mars/sutta3","../mars/sutta4","../mars/sutta7"]
						play_vlc_all_inTime(random.choice(st),mn,'1.50','0.1',1)
									
				else:
					dm = [["../mars/buddhaDhamma","1.50","0.075"],["../mars/12paticca","1.00","0.10"],["../mars/4nt","1.00","0.10"]]
					dmc = random.choices(dm,[3,2,1])[0]
					play_vlc_all_inTime(dmc[0],mn,dmc[1],dmc[2],1)
				...
				singing_bowl(mn)
				ledc('off')
				bell30sec(120,'500','off')
				bell_5minutes()
				wooden_gong_sound(25,'400','r')
				anapanasati_walk(10,"20%")
				musk_walk(10,"20%")
				bell_5minutes()
				ledc('off')
				singing_bowl(2)
				relax_thai()
				play_vlc_inTime('../mars/pope','pope_dhamma',60,'1.00','0.1',0)
				bell('1')
				play_mp3('../mars/monk/rbut.mp3',127,'800')		
	...
	if mm:
		...
		play_vlc_file("../mars/uttayandokmai.mp3",'1.00')
		relax_thai()
		# remind_breathing3(15,'500')
		dokmai(14,'500')
		remind_breathing2(1,'500')
		dokmai(14,'500')
		remind_breathing2(1,'500')
		dokmai(14,'500')
		remind_breathing2(1,'500')
		...
		adjust_volume()
		bell('3')
		play_vlc_file3("../datath/sutta/moggallana.mp3",'2.00')
		mn = x_minutes(5)
		# play_vlc_all_inTime("../mars/dead",mn,'1.50','0.1',1)
		play_vlc_inTime('../mars/dead','dead_dhamma',mn,'1.50','0.1',0)
		# i = weekday()
		# if i == 1:
	...
	if a == 0:
		play_vlc_file("../sound/a-rahung.mp3",'1.00')
		play_vlc_file("../datath/chanting/Bhadhdherattakadha.mp3",'1.00')
		efp3 = ["../datath/chanting/sungkharn.mp3","../datath/chanting/patimok-chanting.mp3"]
		play_vlc_file(random.choice(efp3),'1.00')
		i = weekday() % 2
		if i == 0:
			play_vlc_file("../mars/monk/nava_vinai.mp3",'2.00')
		else:
			play_vlc_file("../mars/monk/nava_dhamma_chanting.mp3",'2.00')
			# play_vlc_file("../mars/monk/pali.mp3",'1.00')
		...
		# efp2 += [["../datath/chanting/ituppajjayata.mp3",1,["../mars/payutto6264/dhamma-2562-09.mp3"]]]
		# efp2 += [["../datath/chanting/dhammaniyam.mp3",1,["../mars/bdd/10024.mp3","../mars/bdd/10025.mp3","../mars/bdd/10026.mp3","../mars/bdd/10027.mp3"]]]
		# efp2c = random.choice(efp2)
		# play_vlc_file(efp2c[0],'1.00')
		# # play_vlc_file('../mars/dhamma-2563-02-cut.mp3','1.00')
		# if efp2c[1] == 1:
		...
		# 	efp3 = ["../datath/chanting/sungkharn.mp3","../datath/chanting/Bhadhdherattakadha.mp3","../datath/chanting/patimok-chanting.mp3"]
		# 	play_vlc_file(random.choice(efp3),'1.00')
	else:
	...
	if m == 1:
		speak("Dhamma Wisdom")
		vlc_one(0,'../mars/wisdom2/',"../mars/log/log_morning.txt",0,'0.1','2.00')
	elif m == 2:
		speak("4 noble truth")
...
def SevenDays_dhamma():
	...
	if i == 1:
		...
		# bd = ["../mars/bdd3","../mars/bdd-3536"]
		# play_vlc_all_inTime(random.choice(bd),mn,'1.50','0.1',1)
		play_vlc_all_inTime("../mars/tripidok",mn,'1.50','0.1',1)
	elif i == 3:
		play_vlc_all_inTime("../mars/payutto6264",mn,'1.50','0.1',1)
	elif i == 4:
		...
		bd = ["../mars/gold","../mars/12paticca"]
		play_vlc_all_inTime(random.choice(bd),mn,'1.50','0.1',1)
	mn  = x_minutes(17)
	dhf = ["../mars/buddhaDhamma/","../mars/tripitaka"]
	i   = i % 2
	play_vlc_all_inTime(dhf[i],mn,'1.50','0.1',1)
...
def my_dhamma():
	speak("Play dhamma that should listen everyday")
	play_vlc_by_list("../mars/247","dhamma_daily",2,0,'0.1','1.00')
...
try:
	...
	with sd.RawInputStream(samplerate=args.samplerate, blocksize = 8000, device=args.device, dtype='int16',
			...
			if a > 0:
				...
				play_vlc_file3("../datath/chanting/paticca.mp3")
			...
			vrun += 'browse chanting mantra say speak stop volume turn on off exit shutdown now thai lyric ip address sutra up down breathing art '
			...
			vrun += 'a alfa b bravo c charlie d delta e echo f foxtrot g golf h hotel i india j juliet k kilo l lima m mike n november o oscar p papa '
			vrun += 'q quebec r romeo s sierra t tango u uniform v victor w whiskey x ray y yankee z zulu '
			vrun += 'letter repeat space spelling speaker noble japanese chinese red green blue yellow hello wisdom continuous human book bell '
							...
							if not focus and len(words) > 1:
								...
								if "say" in words:
									...
									if "two" in words:
										...
										play_vlc_by_list_all("../mars/milin",'0.1','1.50',0)
								...
								if "morning" in words and "meditation" in words:
									...
									if "one" in words:
										...
										play_vlc_by_list_all("../mars/gold",'0.1','1.50',0)
										...
										speak("1 paticca chanting, 2 Sutra, 3 Buddha said, 4 basic chanting, 6 play dhamma")
										clear_q()
									...
									# 	testing_mode1()
									# elif "two" in words:
									# 	play_vlc_by_list_all("../mars/sutta4",'0.1','1.50',0)
									# elif "three" in words:
									# 	play_vlc_by_list_all("../mars/sutta5",'0.1','1.50',0)
									if "four" in words:
										umong_testing_4()
									# elif "two" in words:
									# 	speak("testing 2")
								...
								# for martian monk only 
									...
									if "buddha" in words:
										...
										if "new" in words:
											speak("Play new dhamma talk")
											play_vlc_by_list("../mars/bddnew","dhamma_9_new",3,0,'0.1','1.50')
											# play_vlc_by_list_all("../mars/newbdd",'0.1','1.75',1)
										else:
											speak("play 5 dhamma talk")
											play_vlc_by_list("../mars/bdd","dhamma_9",5,0,'0.1','1.75')
										...
										if "continuous" in words:
											...
											speak("play 2 dhamma talk")
											play_vlc_by_list("../mars/luangpoorian","dhamma_2",2,1)
									elif "three" in words:
										# play_vlc_by_list_all("../mars/char",'0.1','1.50',1)
										speak("Dhamma 3, Luang poo char")
										speakThai_mp3(['หลวง','ปู่','ชา'])
										speak("play 2 dhamma talk")
										play_vlc_by_list("../mars/char","dhamma_3",2,1)
									elif "four" in words:
										...
										play_vlc_by_list_all("../mars/tripidok")
										...
										if "continuous" in words:
											...
											play_vlc_by_list_all("../mars/bhadde",'0.1','1.50',1)
											# vlc_one()
										...
										# 	play_vlc_by_list("../mars/bdd-3536","dhamma_10_bdd",2,0,'0.1','1.50')
										# else:
										# 	speak("Payutto Bikkhu")
										# 	play_vlc_by_list("../mars/payutto6264","dhamma_10_pyt",2,0,'0.1','1.50')
										# speak("Dhamma 10, spoke by Pope")
										# speakThai_mp3(['หลวง','ปู่','ญาณสังวร'])
										# speak("play 4 dhamma talk")
										# play_vlc_by_list("../mars/wisdom2","dhamma_102",4,0,'0.1','2.00')
										play_vlc_by_list("../mars/pope","dhamma_10",2,0,'0.1','1.00')
									elif "noble" in words:
										speak("play 4 dhamma talk")
										play_vlc_by_list("../mars/4nt2","dhamma_noble",2,0,'0.1','1.50')
									elif "new" in words:
										speak("play playlist dhamma talk")
										play_vlc_by_list("../mars/new","dhamma_playlist",1,0,'0.1','1.50')
										# play_four_noble_truth_dhamma()
										...
										speak("Dhamma 12")
										src = ["../mars/surasak","../mars/anan"]
										play_vlc_by_list_all(random.choice(src),'0.1','1.50',1)
